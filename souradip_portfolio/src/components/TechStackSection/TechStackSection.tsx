      ...
      >
        ...
        <div className="flex w-max animate-[marquee_35s_linear_infinite] whitespace-nowrap items-center hover:[animation-play-state:paused] py-1">
          {[...technologies, ...technologies].map((tech, i) => (
            <a 
              key={i} 
              href={`https://www.google.com/search?q=what+is+${encodeURIComponent(tech.name)}`}
              target="_blank"
              rel="noopener noreferrer"
              title={`Click to search "${tech.name}" on Google`}
              className="mx-3 px-5 py-2.5 rounded-full border border-foreground/10 bg-background/80 text-foreground font-medium text-sm flex items-center gap-3 transition-all hover:scale-105 hover:border-primary/50 hover:bg-foreground/5 cursor-pointer shadow-sm group shrink-0"
            >
              <img 
                src={tech.icon} 
                alt={tech.name} 
