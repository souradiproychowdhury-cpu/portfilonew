...
import { HeroVoiceAssistant } from "./HeroVoiceAssistant";
    ...
    <section ref={heroRef} id="hero" className="relative min-h-[100vh] flex flex-col pt-24 sm:pt-28 md:pt-32 overflow-hidden bg-transparent">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full flex-1 flex flex-col md:flex-row items-center justify-center gap-10 md:gap-16 lg:gap-20 pb-12">
        ...
        >
          ...
          >
            {/* AI Voice Assistant Greeting (Male Voice, Hero Section Only) */}
            <HeroVoiceAssistant heroRef={heroRef} />
            ...
            <Badge variant="outline" size="lg" className="gap-2.5 py-1.5 px-4 glass-panel border-foreground/10">
              ...
              <span className="text-xs font-semibold text-muted-foreground">Available for opportunities</span>
            </Badge>
          </motion.div>
          ...
          {/* 2-line Description Greeting */}
          <motion.p 
            className="text-lg md:text-xl text-foreground font-semibold max-w-xl mb-2.5 leading-relaxed w-full"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.45, duration: 0.8 }}
          >
            Computer Science &amp; Engineering student building intelligent AI applications &amp; full-stack web experiences.
          </motion.p>

          <motion.p 
            className="text-base md:text-lg text-muted-foreground max-w-xl mb-8 leading-relaxed w-full"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.5, duration: 0.8 }}
          >
            Passionate about artificial intelligence, interactive computing, and shipping production-grade software with React, Python, and Node.
          </motion.p>
          ...
          >
            ...
            <a 
              ...
              className="rounded-full px-7 h-12 glass-panel text-foreground font-semibold flex items-center gap-2 hover:bg-foreground/10 transition-all hover:-translate-y-1 border border-foreground/10 shadow-sm hover:border-primary/50 cursor-pointer"
            >
              <span>Resume</span>
          ...
          >
            <a href="https://github.com/souradiproychowdhury-cpu" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors hover:-translate-y-1 transform duration-200">
              <Github className="w-5 h-5" />
            </a>
            <a href="https://linkedin.com/in/Souradip-Roy-Chowdhury" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors hover:-translate-y-1 transform duration-200">
              <Linkedin className="w-5 h-5" />
            </a>
            <a href="mailto:souradiproychowdhury@gmail.com" className="text-muted-foreground hover:text-primary transition-colors hover:-translate-y-1 transform duration-200">
          ...
          >
            <div className="flex flex-col h-full bg-card w-full">
              ...
              <div className="relative px-5 pt-7 pb-6 flex flex-col items-center bg-gradient-to-br from-purple-700 via-primary to-indigo-950 text-white overflow-hidden">
                ...
                {/* Profile Photo with animated glow rings */}
                <motion.div
                  className="mt-1 relative w-32 h-32 rounded-full"
                  animate={{ y: [0, -6, 0] }}
                  transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }}
                >
                  {/* Outer spinning gradient ring */}
                  <motion.div
                    className="absolute inset-[-4px] rounded-full bg-gradient-to-tr from-cyan-400 via-primary to-purple-400"
                    animate={{ rotate: 360 }}
                    transition={{ repeat: Infinity, duration: 4, ease: "linear" }}
                  />
                  {/* Pulsing glow behind */}
                  <div className="absolute inset-0 rounded-full bg-primary/50 blur-xl animate-pulse scale-110" />
                  {/* Image container */}
                  <div className="relative w-full h-full rounded-full p-1 bg-gradient-to-tr from-cyan-400 via-primary to-purple-400 shadow-2xl overflow-hidden">
                    <img 
                      src="/avatar.jpg" 
                      alt="Souradip Roy Chowdhury" 
                      className="w-full h-full object-cover object-top rounded-full filter contrast-105 scale-110"
                      loading="eager"
                    />
                  </div>
                  <div className="absolute bottom-1 right-2 w-4 h-4 rounded-full bg-emerald-500 border-2 border-white shadow-md z-10" />
                </motion.div>
              </div>
              ...
              <div className="p-5 flex flex-col items-center text-center bg-card text-card-foreground flex-1 gap-3">
                <div>
                  <h3 className="text-xl font-extrabold tracking-tight text-foreground dark:text-white">Souradip Roy Chowdhury</h3>
                  <div className="inline-flex items-center gap-1.5 mt-1 px-3 py-0.5 rounded-full bg-primary/10 border border-primary/30 text-primary text-xs font-semibold">
                    <span>AI Developer &amp; CSE Student</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2.5 w-full text-left bg-muted/40 p-3 rounded-xl border border-border/50">
                  <div>
                    <span className="text-muted-foreground block text-[9px] uppercase tracking-widest font-semibold">Specialty</span>
                    <span className="font-bold text-foreground text-xs">AI &amp; Web Dev</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block text-[9px] uppercase tracking-widest font-semibold">Location</span>
                    <span className="font-bold text-foreground text-xs">West Bengal, IN</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block text-[9px] uppercase tracking-widest font-semibold">Degree</span>
                    <span className="font-bold text-foreground text-xs">B.Tech CSE 2028</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block text-[9px] uppercase tracking-widest font-semibold">Status</span>
