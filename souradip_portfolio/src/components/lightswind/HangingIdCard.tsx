...
export const HangingIdCard = ({
  ...
  // ── Auto-swing once on initial load for visual discovery ────────────────────
  useEffect(() => {
    if (!autoSwing) return;
    const timer = setTimeout(() => {
      // If user hasn't already grabbed or interacted with the card
      if (!isDraggingRef.current && Math.abs(physRef.current.vel) < 0.1 && Math.abs(physRef.current.angle) < 0.05) {
        physRef.current.angle = 0.28; // starts at ~16 degrees right
        physRef.current.vel = -2.8;   // smooth natural pendulum impulse towards left
        startPhysics();
      }
    }, 700);

    return () => clearTimeout(timer);
  }, [autoSwing, startPhysics]);
